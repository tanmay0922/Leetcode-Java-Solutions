class Solution {
    public int largestOverlap(int[][] img1, int[][] img2) {
        int R1 = img1.length; //length
        int R2 = img2.length; //length 
        int C1 = img1[0].length; //first index is zero
        int C2 = img2[0].length;  //first index is zero
        int max = Integer.MIN_VALUE;  //munimium integer value is stored
		
		//Slide one image on other image.
        for(int r=0; r<R1+R2-1; r++){  //loop till length of both img1 and img2
